  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Header */}
        <div
          className="px-7 pt-8 pb-5 relative"
          style={{ background: "linear-gradient(135deg, #0e3256 0%, #1b5fa0 60%, #3e8ad6 100%)" }}
        >
          {/* Close button */}
          <button
            onClick={onClose}
            className="absolute top-4 right-4 w-9 h-9 rounded-full flex items-center justify-center text-white transition-all duration-200 hover:bg-white/20 hover:scale-110"
            aria-label="Close"
          >
            <Icons.Close />
          </button>
          ...
          {/* Badge */}
          <span className="inline-block bg-white/15 border border-white/25 text-white text-xs font-bold px-3 py-1.5 rounded-full mb-3">
          ...
          <p className="text-blue-200 text-sm leading-relaxed">
            {subtitle ?? "Find out exactly where your practice is losing revenue — no commitment required."}
          </p>

          {/* Trust badges */}
          <div className="flex flex-wrap gap-2 mt-4">
            {["🔒 HIPAA Compliant", "⚡ 7–10 Day Onboarding", "📊 No Setup Fees"].map((b, i) => (
              <span
                key={i}
                className="text-xs font-semibold bg-white/10 border border-white/15 text-white px-2.5 py-1 rounded-full"
              >
                {b}
              </span>
            ))}
        ...
        <div className="px-7 py-6">
          ...
          >
            <div className="grid grid-cols-2 gap-3">
              <PopupInput type="text" placeholder="First Name" label="First Name" />
              <PopupInput type="text" placeholder="Last Name" label="Last Name" />
            </div>
            <PopupInput type="email" placeholder="Email Address" label="Email Address" />
            <PopupInput type="tel" placeholder="(555) 123-4567" label="Phone Number" />
            <PopupSelect label="Practice Specialty" options={specialtyNames} />
            <PopupTextarea placeholder="Tell us about your billing needs..." label="Message (Optional)" />

            <button
              type="submit"
              className="w-full text-white py-3.5 rounded-xl font-bold text-base transition-all duration-300 hover:-translate-y-0.5 hover:shadow-xl group relative overflow-hidden"
...
// ── Styled textarea ────────────────────────────────────────────────────────
function StyledTextarea({ placeholder, label }: { placeholder: string; label?: string }) {
  return (
    <div>
      {label && <label className="block text-xs font-semibold text-gray-700 mb-1.5">{label}</label>}
      <textarea
              ...
              >
                <div className="bg-white rounded-3xl p-7 sm:p-8 shadow-2xl border border-white/10 relative overflow-hidden">
                  ...
                  <form className="space-y-3.5" onSubmit={(e) => e.preventDefault()}>
                    ...
                    <StyledSelect options={specialtyNames} />
                    <StyledTextarea placeholder="How can we help your practice?" />
                    <button
                      type="submit"
